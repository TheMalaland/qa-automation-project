...
import { ReporterRuntime, createDefaultWriter, ensureSuiteLabels, getEnvironmentLabels, getFrameworkLabel, getLanguageLabel, getPackageLabel } from "allure-js-commons/sdk/reporter";
...
export class AllureMochaReporter extends Mocha.reporters.Base {
  constructor(runner, _opts) {
    ...
    _defineProperty(this, "onTest", test => {
      ...
      var initialLabels = [getLanguageLabel(), getFrameworkLabel(this.getFrameworkName()), getHostLabel(), getThreadLabel(this.getWorkerId())];
      var metaLabels = getAllureMetaLabels(test);
      var labels = globalLabels.concat(initialLabels, metaLabels);
      if (test.file) {
        var packageLabel = getPackageLabel(test.file);
    ...
    _defineProperty(this, "onFailed", (_, error) => {
      ...
      this.runtime.updateTest(this.currentTest, r => {
        ...
        r.statusDetails = _objectSpread(_objectSpread({}, r.statusDetails), getMessageAndTraceFromError(error));
      });
    });
    ...
    if (!isInWorker && extraReporters) {
      _classPrivateFieldSet(_extraReporters, this, enableExtraReporters(runner, _opts, extraReporters));
    }
  }
  done(failures, fn) {
    this.runtime.writeEnvironmentInfo();
    this.runtime.writeCategoriesDefinitions();
    doneAll(_classPrivateFieldGet(_extraReporters, this), failures, fn);
  }
}
